...
def fit(model, epochs, train_loader, dev_loader):
    ...
    for epoch in range(epochs):
        ...
        print(
            "dev_loss = {:.4f} dev_acc = {:.4f}".format(
                *perf(model, dev_loader, criterion)
            )
        )
...
def read_corpus(
    filename, wordvocab, tagvocab, max_len, batch_size, train_mode=True, batch_mode=True
):
    if train_mode:
...
def get_args():
    parser = argparse.ArgumentParser(description="Training RNN POS-tagger")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Chemin vers le fichier d'entraînement (CoNLL-U)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Chemin vers le fichier de développement (CoNLL-U)",
    )

    parser.add_argument(
        "-e", "--epochs", type=int, default=15, help="Nombre d'époques d'entraînement"
    )
    ...
    parser.add_argument(
        "-bs", "--batch_size", type=int, default=32, help="Taille des batchs"
    )
...
if __name__ == "__main__":
    ...
    train_loader, wordvocab, tagvocab = read_corpus(
    ...
    # Chargement du corpus de dev
    dev_loader, _, _ = read_corpus(
    ...
    torch.save(
        ...
        f"models/model_e{args.epochs}_l{args.max_len}_bs{args.batch_size}.pt",
    )
