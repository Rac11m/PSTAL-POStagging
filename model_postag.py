...
class RNN_postag(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, output_size: int, num_embeddings: int, embedding_dim: int, PAD_ID: int):
        super().__init__()
        self.PAD_ID = PAD_ID
        self.embed = nn.Embedding(num_embeddings, embedding_dim, padding_idx=PAD_ID)
        self.gru = nn.GRU(input_size, hidden_size, batch_first=True, bias=False)
